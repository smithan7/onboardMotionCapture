...
while ~isDone(videoFReader) && index < 900
    ...
    if isMarkers
        ...
        if iter > 0
            l_median = sumPN / iter;
            qLoc(3) = l_median/85;
            raw_z(nFrame) = qLoc(3); % Storing raw angle before Kalman Filter for debug use
        end
...
x2(:,2) = x2(:,2) + mE(2);
%x2(:,3) = x2(:,3) + mE(3);
x2(:,3) = -(x2(:,3) - mean(x2(1:899,3))) + mean(o2(1:899,3));
raw_z = -(raw_z*.3048 - mean(raw_z(1:899)*.3048)) + mean(o2(1:899,3));
...
figure
set(gca,'fontsize',16,'fontname','Times')
plot(time, x2(:,3),time, o2(:,3),time, raw_z, 'linewidth',2)
xlabel('Time (s)')
ylabel('Position Z (m)')
legend('Raw', 'Kalman')
grid on